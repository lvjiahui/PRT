...
std::unique_ptr<std::vector<Eigen::Array3f>> ProjectEnvironment_Par(
    ...
    std::unique_ptr<std::vector<Eigen::Array3f>> coeffs(
        new std::vector<Eigen::Array3f>());
    coeffs->assign(GetCoefficientCount(order), Eigen::Array3f(0.0, 0.0, 0.0));
    std::mutex coeffs_mutex;

    std::vector<int> lines;
    for (int t = 0; t < env.height(); t++) lines.push_back(t);
    std::for_each(std::execution::par, lines.begin(), lines.end(),
        [&](int t) {
        std::vector<Eigen::Array3f> t_coeffs(GetCoefficientCount(order), Eigen::Array3f(0.0, 0.0, 0.0));
        Eigen::Array3f color;
        double theta = ImageYToTheta(t, env.height());
        double weight = pixel_area * sin(theta);
        for (int p = 0; p < env.width(); p++) {
            ...
            for (int l = 0; l <= order; l++) {
                for (int m = -l; m <= l; m++) {
                    int i = GetIndex(l, m);
                    double sh = EvalSH(l, m, phi, theta);
                    t_coeffs[i] += sh * weight * color.array();
                }
            }
        }
        std::lock_guard<std::mutex> guard(coeffs_mutex);

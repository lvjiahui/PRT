...
void main()
{
    ...
    if(diffuse && !metal){
        if(sh)
            // diffuse_color = Sh_color; //sh contains 1/PI
            diffuse_color = SH_Irad(envRotate*vec4(N, 1.0), WorldPos) / PI;
        else
            diffuse_color = texture(irradiance, vec3(envRotate*vec4(N, 1.0))).rgb / PI;

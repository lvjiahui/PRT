...
GLuint equirectangular_to_cubemap(SkyBox &skybox, size_t w, size_t h){
    // setup framebuffer
    // ----------------------
    GLuint captureFBO;
    GLuint captureRBO;
    ...
    for (GLuint i = 0; i < 6; ++i)
    {
        ...
        skybox.setup_rectangle(captureProjection, captureViews[i]);
        skybox.render();
    }
...
GLuint irradiance_map(SkyBox &skybox, size_t w, size_t h){
    // setup framebuffer
    // ----------------------
    GLuint captureFBO;
    GLuint captureRBO;
    ...
    for (GLuint i = 0; i < 6; ++i)
    {
        glFramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT0, GL_TEXTURE_CUBE_MAP_POSITIVE_X + i, envCubemap, 0);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        skybox.setup_irradiance(captureProjection, captureViews[i]);
        skybox.render();
    }
    glBindFramebuffer(GL_FRAMEBUFFER, 0);
    glViewport(m_viewport[0], m_viewport[1], m_viewport[2], m_viewport[3]); 
